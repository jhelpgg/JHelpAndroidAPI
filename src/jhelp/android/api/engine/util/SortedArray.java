...
public final class SortedArray<TYPE>
{
	 ...
	 */
	@SuppressWarnings("unchecked")
	public SortedArray(final Class<TYPE> typeClass, final Comparator<TYPE> comparator, final int initialSize,
			final boolean unique)
	{
		...
		if(comparator == null)
		{
			if(Utilities.isSubTypeOf(typeClass, Comparable.class) == true)
			{
				this.comparator = new ComparatorNatural<TYPE>();
			}
			else
			{
				throw new IllegalArgumentException("comparator is null and the type class " + typeClass.getName()
						+ " is not comparable");
			}
		}
